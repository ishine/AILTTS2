...
class KernelPredictor(torch.nn.Module):
    ...
    #* Mel(cond)을 입력으로 받아 하나의 LVC block의 kernel, bias들을 예측한다. 최종 Output의 구체적인 모양은 논문에 따로 적어두었음. 
    def __init__(
            self,
            cond_channels,          #* 100
            conv_in_channels,       #* 16 or 32
            conv_out_channels,      #* 32 or 64
            conv_layers,            #* 4 (len(dilations))
            conv_kernel_size=3,     
            kpnet_hidden_channels=64,
            kpnet_conv_size=3,      #* 1
            kpnet_dropout=0.0,
            kpnet_nonlinear_activation="LeakyReLU",
            kpnet_nonlinear_activation_params={"negative_slope":0.1}, #* 0.2
        ):
        '''
        Args:
        ...
        self.conv_in_channels = conv_in_channels    #* 16 or 32
        self.conv_out_channels = conv_out_channels  #* 32 or 64
        self.conv_kernel_size = conv_kernel_size    #* 3
        self.conv_layers = conv_layers              #* 4 (len(dilations))

        kpnet_kernel_channels = conv_in_channels * conv_out_channels * conv_kernel_size * conv_layers   #! l_w: 16*32*3*4
        kpnet_bias_channels = conv_out_channels * conv_layers                                           #! l_b: 32*4
    ...
    def forward(self, c):
        ...
        #* (in_ch: 100, out_ch: 64, ker_size: 5)
        c = self.input_conv(c)    #* (64, 5)
        for residual_conv in self.residual_convs:
            #* residual_conv: (Conv1d+LReLU) X 2
            residual_conv.to(c.device)
            c = c + residual_conv(c)
        k = self.kernel_conv(c) #* in_ch: 64, out_ch: C_g*2C_g*conv_kernel_size(3)*LVC_layers(4)
                                #* conv_kernel_size: kernel predictor가 아닌 LVC의 kernel size
        b = self.bias_conv(c)   #* in_ch: 64, out_ch: 2C_g * LVC_layers(4)
...
class LVCBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.kernel_predictor = KernelPredictor(
            ...
            conv_in_channels=in_channels,       #* 16 or 32
            conv_out_channels=2 * in_channels,  #* 32 or 64
